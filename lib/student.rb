class Student
  ...
  def self.first_X_students_in_grade_10(x) #x must be small because X isn't allowed.
    sql = <<-SQL
    SELECT *
    FROM students WHERE grade = 10 LIMIT ?
    SQL
  ...
  def self.first_student_in_grade_10
    sql = <<-SQL
    SELECT *
    FROM students WHERE grade = 10 ORDER BY id ASC LIMIT 1
    SQL
